---
title: 'Stan notebook for logistic regression - checking for reproducibility with
  Cycle 1 Data '
...
download.file("http://davidrand-cooperation.com/s/Rand2011PNAS_data_and_code-pi6b.zip",
              temp, mode="wb")
pilotdata <- read.table(unz(temp,"Rand2011PNAS_cooperation_data.txt"),
                          sep="\t",skip=0,header=T)
unlink(temp)
#cooperation <- read.table("data/Rand2011PNAS_cooperation_data.txt", sep="\t",skip=0, header=T)

### This dataset contains the cooperation decisions at the individual level
###    for each cooperation round
exp1_cooperation <- read.csv(url("https://raw.githubusercontent.com/gallup/NGS2/master/cooperation_exp1.csv"),
                             header = TRUE,
                             sep = ',')

### This dataset contains the connection decisions at the individual level
###   for each rewiring round
exp1_rewire <- read.csv(url("https://raw.githubusercontent.com/gallup/NGS2/master/rewire_exp1.csv"),
                             header = TRUE,
                             sep = ',')

cooperation <- exp1_cooperation

### Update names in experiment 1 dataset so that they match the names of
###   the pilot dataset
names(cooperation)[names(cooperation) %in%
    c("round","pid","action")] <- c("round_num",
                            "playerid","decision..0.D.1.C.")
cooperation$sessionnum <- unlist(lapply(1:nrow(cooperation), function(i){
                which(cooperation$session[i]==levels(cooperation$session)) }))
...
for (i in 1:num.boot.sets){
  samp[i,] <- sample(unique(cooperation$sessionnum),
                     length(unique(cooperation$sessionnum)), replace=TRUE)
